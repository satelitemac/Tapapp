...
st.markdown("""<style>
    ...
    .bio-box { background: rgba(255,255,255,0.03); padding: 1.2vh; border-radius: 10px; color: #ccc; border: 1px solid #222; font-size: 1.35vh; margin-bottom: 1vh; min-height: 15vh; overflow-y: auto; }
...
if d:
    ...
    with c3:
        ...
        # --- BLOQUE RÍGIDO DERECHO (650px - AQUÍ OCURRE LA MAGIA) ---
        with st.container(height=650, border=False):
            if st.session_state.panel_derecho_contenido:
                # MODO POPUP GRANDE
                st.markdown(f"""<div class="expanded-right-panel"><div class="expanded-right-title">🔍 {st.session_state.panel_derecho_titulo}</div><div style="font-size: 2.2vh;">{st.session_state.panel_derecho_contenido}</div></div>""", unsafe_allow_html=True)
                ...
                # Créditos (Limpios y sin errores)
                ...
                if credits:
                    ...
                    if c_bc.button("👁️", key="b2"): 
                        content = "".join([f"<p>{c['role']}: {c['name']}</p>" for c in credits])
                        st.session_state.panel_derecho_contenido = content
                        st.session_state.panel_derecho_titulo = "CREDITS"
                        st.rerun()
                    cred_preview = "".join([f'<div class="credit-item">{c["role"]}: {c["name"]}</div>' for c in credits[:3]])
